...
void save_Bus_info(Event event) {
	...
	case 1: {
		fileout.open("Bus_number_one_optimization.txt", ios::app);
	}break;
	case 2: {
		fileout.open("Bus_number_two_optimization.txt", ios::app);
	}break;
	case 3: {
		fileout.open("Bus_number_three_optimization.txt", ios::app);
	}break;
	case 4: {
		fileout.open("Bus_number_four_optimization.txt", ios::app);
	}break;
	case 5: {
		fileout.open("Bus_number_five_optimization.txt", ios::app);
	}break;
	}
...
void save_Stop_info(int hourly_time, int stop_number, double wait_number) {
	ofstream fileout;
	fileout.open("Stop_waiting_length_hourly_optimization.txt", ios::app);
...
void save_Stop_Waiting_info(vector<unordered_map<double, int> > &wait_sequence_counter) {
	ofstream fileout;
	fileout.open("Bus_Stop_waiting_Observe_optimization.txt");
	for (int i = 0; i < 15; i++) {
...
void save_total_average_stop_info(int stop_number, double max, double min, double total_average) {
	ofstream fileout;
	fileout.open("Stop_total_average_waiting_info_optimization.txt", ios::app);
